...
var svg = d3.select('body').append("svg")
  ...
  .attr('height', height)
  .append('g')
  .attr('transform', 'translate(' + (offset_x+width-150) / 2 + ',' + (offset_y+height) / 2 + ')');
...
var tooltip_fixed = d3.select("body").append("div").attr("class","tooltip_fixed").style("opacity",0);
...
var legend= d3.select("svg")
  ...
  .append("g")
    .attr("transform", function(d,i) { //Aligning the legend (each of the circles and the text) to the right side of the sunburst
        ...
        var x = (width-125)+offset_x;
        var y = i * legend_height+((height-400)/2)+offset_y;
